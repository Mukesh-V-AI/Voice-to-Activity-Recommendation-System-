...
st.set_page_config(
    page_title="🎙️ Voice Activity Recommender",
    page_icon="🎙️",
    layout="wide",
...
class ActivityRecommenderUI:
    ...
    def get_text_recommendations(self, text: str) -> Dict[str, Any]:
        """Get recommendations from text input."""
        try:
    ...
    def get_audio_recommendations(self, audio_bytes: bytes, filename: str) -> Dict[str, Any]:
        """Get recommendations from audio file."""
        try:
...
def render_recommendation_card(rec: Dict[str, Any], index: int):
    ...
    with st.container():
        ...
        with col3:
            ...
            st.markdown(f"<div style='color: {score_color}'>Similarity: {rec['similarity']:.2f}</div>", 
                       unsafe_allow_html=True)

        st.divider()

def render_intent_analysis(intent_data: Dict[str, Any]):
    """Render intent analysis visualization."""
    ...
    if sentiment:
        ...
        with col2:
            ...
            st.metric("Subjectivity", obj_label, f"{subjectivity:.2f}")
...
def main():
    ...
    with st.sidebar:
        ...
        if api_healthy:
            ...
            st.warning("Please ensure the FastAPI backend is running on localhost:8001")
    ...
    # Main tabs
    tab1, tab2, tab3, tab4 = st.tabs(["🎯 Get Recommendations", "📊 Analytics", "📋 Browse Activities", "ℹ️ About"])
        ...
        if input_method == "💬 Text Input":
            ...
            with col1:
                if st.button("😰 Feeling Stressed"):
                    st.session_state.text_input = "I feel stressed and anxious, need something calming for 20 minutes"

            with col2:
                if st.button("⚡ Want Energy"):
                    st.session_state.text_input = "I feel energetic and want something physical for an hour"
        ...
        if activities_data:
            ...
            if activities:
                ...
                if selected_category != "All":
                    filtered_activities = [a for a in filtered_activities if a['category'] == selected_category]

                if selected_mood != "All":
                    filtered_activities = [a for a in filtered_activities if a['mood'] == selected_mood]

                filtered_activities = [a for a in filtered_activities if a['time_minutes'] <= max_time]
